...
- **监考事件复核中心**：考试中出现切屏、退出全屏、离开页面时前端自动上报事件（30 秒去重窗口内同人同类只保留一条）；教师在复核中心按待处理/已确认/已忽略筛选自己考试的事件并填写处理结论，结论提交后不可再改；管理员可查看全部处理记录。

## 监考事件复核中心

- **事件触发**：考试页面监听切屏（`visibilitychange`）、退出全屏（`fullscreenchange`）、离开页面（`pagehide`/`beforeunload`）三类行为并自动上报；离开页面上报使用 `keepalive` 请求，随后按既有规则自动交卷。
- **重复上报**：同一考试场次、同一事件类型在 30 秒窗口内只保留第一条，后续上报返回 `deduplicated: true` 并指向已有事件；去重由 `(attempt_id, type, window_bucket)` 唯一索引在数据库层兜底，并发同时上报也不会重复入库。
- **处理流程**：事件初始为「待处理」，教师只能看到并处理自己创建考试的事件，处理时选择「确认违规」或「忽略」并填写结论；结论一旦提交不可再修改（重复提交返回 409）。
- **权限边界**：学生仅可上报；教师数据范围限定本人考试；管理员可查看全部事件与处理记录；越权访问返回 403。
...
| 方法 | 路径 | 说明 | 权限 |
| --- | --- | --- | --- |
| POST | `/api/v1/proctor-events` | 上报监考事件（30 秒窗口去重） | 学生 |
| GET | `/api/v1/proctor-events` | 事件列表（按状态/类型/考试筛选） | 教师（本人考试）/管理员（全部） |
| GET | `/api/v1/proctor-events/:id` | 事件详情 | 教师（本人考试）/管理员 |
| POST | `/api/v1/proctor-events/:id/review` | 提交处理结论（确认/忽略，提交后不可改） | 教师（本人考试）/管理员 |
